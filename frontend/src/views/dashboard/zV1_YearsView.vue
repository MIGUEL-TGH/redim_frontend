...
export default {
  ...
  methods: {
    ...
    // -------------------------------------------------------------------------------------------------------------
    async CRUD_DOMS_V1 (value) {
      // console.log('CRUD_DOMS_V1 -->', value)
      const CRUD_ = {
        saved_item: async () => {
          const newItem = {
            ...this.forms,
          ...
          await this.dataTable.items.unshift(newItem)
          this.$store.dispatch('success', { message: '¡Registro guardado correctamente!' })
        },
        updated_item: async () => {
          const index = this.dataTable.items.findIndex(element => { return element.id === value.id })
          if (index === -1) return
          // --------------------------- opcion 1-----------------------------------------------------------------------
          // this.dataTable.items[index].name = this.forms.name

          // --------------------------- opcion 2-----------------------------------------------------------------------
          const { status, ...filteredData } = this.forms // excluye el campo 'status' de filteredData
          Object.keys(filteredData).forEach(key => {
            const itemValue = this.forms[key]
            if (Object.prototype.hasOwnProperty.call(this.dataTable.items[index], key)) {
              this.dataTable.items[index][key] = itemValue
            }
          })

          this.$store.dispatch('success', { message: '¡Registro actualizado correctamente!' })
        },
        status_updated: async () => {
          const index = this.dataTable.items.findIndex(element => { return element.id === value.id })
          if (index === -1) return

          this.dataTable.items[index].status = value.status
          this.$store.dispatch('success', { message: '¡Estatus actualizado correctamente!' })
        }
      }
      CRUD_[value.task] ? CRUD_[value.task]() : console.log('¡CRUD_DOMS_V1 not found!')
    },
    async CRUD_ELEMENT_V1 (value) {
      try {
        ...
        const CRUD_ = {
          send_item: async () => {
            ...
            dataSend.task = (this.params.id === 0) ? 'insert' : 'update'
          },
          status_item: async () => {
            dataSend.task = 'update_status'
            dataSend.params = { id: value.id }
            dataSend.params.status = (value.status) ? 1 : 0
          }
        }
        ...
        if (response.data.status === 200) {
          this.CRUD_DOMS_V1(response.data.result)
          this.reset({ task: 'close_item' })
        }
      } catch (error) {
        console.log(error.response.data)
